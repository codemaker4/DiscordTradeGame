...
function makeInvConfNum() {
  ...
  for (var i = 0; i < inventory.length; i++) {
    sum += (inventory[i]+2)*(i+4);
  }
  for (var i = 0; i < buildings.length; i++) {
    sum += (buildings[i]+5)*(i+3);
  }
  return (((sum + invconfSeed) * invconfSeed)%5.3713);
}

function checkInvConfNum() {
  return makeInvConfNum() == invconf;
}
...
function redrawInf() {
  ...
  string += "</ul><h3 style='margin-bottom:1vh;'>instellingen:</h3>" // titel voor de instellingen knoppen onderaan het scherm
  string += "<p style='font-size:3vh;margin:0;'><b>versie: " + Version + "</b></p>"
